...
cache_t* new_cache(MMP_WCHAR* url, int sync)
{
    int i;
    ...
    if (sync)
    {
        c->fd = (int)PalWFileOpen(c->url, PAL_FILE_WBP, MMP_NULL, MMP_NULL);
#ifdef DEBUG_WRITE_FAIL
        printf("fd(%X)\n", c->fd);
#endif
        if (!c->fd)
        ...
        PalWFileOpen(c->url, PAL_FILE_WBP, _file_open_callback, &c->fd);
...
void free_cache(cache_t *c)
{
    if (c)
    {
        if (c->fd)
        {
            ...
            _PrintFileName(c->url);
            PalWFileDelete(c->url, _file_callback, &done);
            while (!done)
            {
#ifdef DEBUG_WRITE_FAIL
                trac("");
...
#ifdef _LOG_
        PalWcscpy(filename, c->url);
        filename_len = PalWcslen(filename);
        filename[filename_len] = '_';
        filename[filename_len+1] = 0;
        f = PalWFileOpen(filename, PAL_FILE_WB, MMP_NULL, MMP_NULL);
#endif
