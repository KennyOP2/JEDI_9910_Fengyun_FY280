...
_PrintFileName(
    MMP_WCHAR* filename)
{
    MMP_CHAR sfilename[255];
    MMP_INT  i = PalWcslen(filename);

    for (; i >=0; --i)
    {
        sfilename[i] = (MMP_CHAR)filename[i];
    }
    printf("filename: %s\n", sfilename);
}
#else
    ...
    RECORD_MODE*    ptRecMode)
{
    do
    {
        ...
        mectx->ori_filename     = PalWcsdup(filename);
        mectx->fragment_count   = 0;
        mectx->a_pts            =
        mectx->v_pts            = -1;
        mectx->timeunit[AUDIO]  =
        mectx->timeunit[VIDEO]  = 0;
        extension = PalWcsrchr(mectx->ori_filename, L'.');
    ...
    double      pts)
{
     ...
     && pts > (mectx->a_pts + mectx->base_pts))
    {
        ...
        if (mectx->tAudioInfo.audioEncoderType == MMP_MP2_ENCODE)
        {
    ...
    AUDIO_INFO *ptInfo)
{
    ...
    // ============= AUDIO ===============
    mectx->_sh_audio.format     = (ptInfo->audioEncoderType == MMP_MP2_ENCODE)
                                ? 0x00000055
                                : MKTAG('M', 'P', '4', 'A');
...
{
    MMP_WCHAR numStr[10];
    PalWcscpy(mectx->out_filename, mectx->ori_filename);
    if (mectx->fragment_count > 0)
    {
        smtkItow(mectx->fragment_count, numStr);
        PalWcscat(mectx->out_filename, L"_");
        PalWcscat(mectx->out_filename, numStr);
    }
    PalWcscat(mectx->out_filename, L".mp4");
