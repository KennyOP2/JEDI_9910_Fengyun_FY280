...
stream_t* open_stream_full(const MMP_WCHAR* filename, int sync) {
    int i,r;
    const stream_info_t* sinfo;
    stream_t* s;
...
stream_t* open_output_stream(const MMP_WCHAR* filename, int sync) {
    if (!filename) {
...
void free_stream(stream_t *s, int sync) {
    ...
    if (s->fd > 0) {
        //PalFileClose((PAL_FILE*)s->fd, MMP_NULL);
#ifdef ASYNC_FILE_HANDLE
        int done = 0;
        if (sync)
        {
        ...
        {   // delete empty file
            ...
            if (sync)
            {
                ...
                PalWFileDelete(s->url, _file_callback, &done);
                while (!done)
                {
#ifdef DEBUG_WRITE_FAIL
                    trac("");
                ...
                PalWFileDelete(s->url, _file_callback, MMP_NULL);
#else
            PalWFileDelete(s->url, MMP_NULL, MMP_NULL);
#endif
        }
    }
