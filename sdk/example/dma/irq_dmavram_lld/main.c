...
static MMP_EVENT isrEvent = MMP_NULL;
...
#define DMA_LLD_MAX_SIZE    300
    ...
    while(totalSize > 0)
    {
		...
		#if defined(USE_IRQ) 
		sysLLD[lldSize].control |= LLD_INTR_TC_MSK; /** disable tc interrupt */
		#endif
        /** current tx size */
	...
	#if defined(USE_IRQ) 
	sysLLD[lldSize].control &= ~LLD_INTR_TC_MSK; /** enable tc interrupt at last transfer */
	#endif
...
MMP_INT DMA_Vram2Vram(MMP_UINT32 srcOffset, MMP_UINT32 dstOffset, MMP_UINT32 burstSize)
{
    ...
    // Process DMA.
    {
		...
		#if defined(USE_IRQ)
		mmpDmaRegisterIsr(dmaCtxt, isr, MMP_NULL);
		#endif
        ...
        if(result)
            goto end;
		#if defined(USE_IRQ)
	    result = SYS_WaitEvent(isrEvent, IRQ_TIMEOUT);
		if(result)
		{
