...
#include "common.h"
#include "debug.h"
#include "pal/pal.h" // for PalSleep()
...
#ifndef HCC_UNICODE
#if (!F_LONGFILENAME)
	return (char*)("HCC_FAT ver:3.39");
#else
	return (char*)("HCC_FAT_LFN ver:3.39");
#endif
#else
	return (char*)("HCC_FAT_LFN_UNI ver:3.39");
#endif
}
...
void _f_initentry(F_DIRENTRY *de,const char *name,const char *ext)
{
	...
	_f_set16bitl(de->cdate,f_getdate());  /* if there is realtime clock then creation date could be set from */
	_f_set16bitl(de->ctime,f_gettime());  /* if there is realtime clock then creation time could be set from */
}
					...
					if (f->mode==FN_FILE_RD)
					{
						...
						if (!f->startcluster)
						{
							if (file->startcluster)
							{
								...
								f->abspos=(unsigned long)(-(vi->bootrecord.bytes_per_sector)); /* forcing seek to read 1st sector! abspos=0; */
...
int _f_emptywritecache(F_VOLUME *vi, FN_FILEINT *f)
{
	...
	while (i<f->WrDataCache.size)
	{
		for (j=i+1;j<f->WrDataCache.size && ((f->WrDataCache.pos+(j-1))->sector)+1==(f->WrDataCache.pos+j)->sector && (unsigned)(j-i)<256;j++)
		  ;
	...
	if (ret)
	{
		...
		for (;i<f->WrDataCache.size;i++)
		{
			...
			if (bad)
			{
				...
				for (j=f->WrDataCache.size-1;j>i;j--)
				{
...
int _f_emptywritebuffer(F_VOLUME *vi,FN_FILEINT *f,char *ptr)
{
	...
	if (ptr)
	{
		if (f->WrDataCache.size==WR_DATACACHE_SIZE)
		{
			ret=_f_emptywritecache(vi,f);
			if (ret) return ret;
		}

		if (f->WrDataCache.size==0) f->WrDataCache.ptr=ptr;

		(void)_memcpy(f->WrDataCache.pos+(f->WrDataCache.size++),&(f->pos),sizeof(F_POS));
	}
	else
	{
...
int _f_fseek(F_VOLUME *vi,F_MULTI *fm,FN_FILEINT *f,unsigned long offset)
{
...
int _f_updatefileentry (F_VOLUME *vi, FN_FILEINT *f, int remove)
{
	...
	_f_set16bitl(de->cdate,f_getdate());  /* if there is realtime clock then creation date could be set from */
	_f_set16bitl(de->ctime,f_gettime());  /* if there is realtime clock then creation time could be set from */
	_f_set16bitl(de->lastaccessdate,f_getdate());  /* if there is realtime clock then lastaccess date could be set from */
	...
	case FN_SEEK_CUR:
		{
			...
			return F_SETLASTERROR(_f_fseek(vi,fm,f,f->abspos+f->relpos+offset));
		}
	case FN_SEEK_END:
		{
	...
	case FN_SEEK_SET:
		{
			...
			return F_SETLASTERROR(_f_fseek(vi,fm,f,(unsigned long)offset));
		}
	default:
...
//printf("f_enterFS in\n");
	...
	if (!fnGetTask(0))
	{ 
	   //printf("taskid is exist \n");
	   return F_NO_ERROR; /* already in */
	}  
#endif
	...
	for (;;)
	{
		//printf("wait until global mutex is released\n");
...
void _fn_updateseekpos(FN_FILEINT *f)
{
    ...
    else if (pos >= F_MAXSEEKPOS)
    {
        for (dst=0; dst<(F_MAXSEEKPOS>>1); dst++)
        {
            src=dst*2+1;
            f->seekpos[dst]=f->seekpos[src];
            f->seekprev[dst]=f->seekprev[src];
        }

        for (src=(F_MAXSEEKPOS>>1); src<F_MAXSEEKPOS; src++)
        {
            f->seekpos[src] = -1;
            f->seekprev[src] = -1;
        }
