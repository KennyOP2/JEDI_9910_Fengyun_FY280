...
#else
	#define F_MAXLNAME	512			/* maximum length of long filename */
	typedef struct
	{
		int drivenum; 				/*  drive number 0-A 1-B 2-C  */
...
typedef struct
{
	...
	F_NAME findfsname;		   	/* find properties */
	F_POS pos;					/* position of the current list */
    F_POS oripos;				/* origan position of the current list */
} FN_WFIND;
#endif
...
#define f_wfindbypos(find) fm_wfindbypos(find)
...
extern int fm_wfindbypos(FN_WFIND *find);
