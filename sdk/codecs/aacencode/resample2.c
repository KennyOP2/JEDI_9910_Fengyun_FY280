  ...
  else
  {
    for(dst_index=0; dst_index < dst_size; dst_index++)
    {
        ...
        else
        {
...
#else
#if RESAMPLE_FILTER_LENGTH==8
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index] ), "r"((FELEM2)filter[0]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 1] ), "r"((FELEM2)filter[1]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 2] ), "r"((FELEM2)filter[2]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 3] ), "r"((FELEM2)filter[3]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 4] ), "r"((FELEM2)filter[4]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 5] ), "r"((FELEM2)filter[5]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 6] ), "r"((FELEM2)filter[6]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 7] ), "r"((FELEM2)filter[7]));
            asm volatile ("l.macrc %0, 15" : "=r"(val));
#else
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index] ), "r"((FELEM2)filter[0]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 1] ), "r"((FELEM2)filter[1]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 2] ), "r"((FELEM2)filter[2]));
			...
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 6] ), "r"((FELEM2)filter[6]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 7] ), "r"((FELEM2)filter[7]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 8] ), "r"((FELEM2)filter[8]));
			asm volatile ("l.mac %0,%1" : : "%r"((int)src[sample_index + 9] ), "r"((FELEM2)filter[9]));
            asm volatile ("l.macrc %0, 15" : "=r"(val));
#endif
		    dst[dst_index] = CLIPTOSHORT(val);
#endif
        }
