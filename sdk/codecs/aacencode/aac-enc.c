...
uint8_t* inputBuf;
#else
uint8_t inputBuf[MAX_SUPPORT_CHANNELS*2*1024];
#endif
int16_t convertBuf[MAX_SUPPORT_CHANNELS*1024];
    ...
    if (gAACAudioEncode.nInputBufferType==2 && gAACAudioEncode.nEnableMixer ==0){
     ...
     //   releasePCMBuffer();
        ...
        if (bNoMixer)
        {
            ...
            {
                if (nTemp+wavReadIdx<=gAACAudioEncode.nBufferLength){
                    memcpy(buf,pSrcbuf,nTemp);
                } else {
                    //printf("[AAC Enc] memcpy \n");
                    memcpy(buf,pSrcbuf,gAACAudioEncode.nBufferLength-wavReadIdx);
                    pSrcbuf = (short*) &gAACAudioEncode.pInputBuffer[0];
                    memcpy(&buf[gAACAudioEncode.nBufferLength-wavReadIdx],pSrcbuf,nTemp-(gAACAudioEncode.nBufferLength-wavReadIdx));
                }
                // volume control
                ControlVolume(buf, gAACEncoder.nb_channels * 1024);
            }
        }
        else if (bNoI2S)
        {
            if (gAACAudioEncode.nSampleSize==16)
            {
                if (nTemp+mixReadIdx<=gAACAudioEncode.nMixBufferLength){
                    memcpy(buf,pMixbuf,nTemp);
                } else {
                    //printf("[AAC Enc] memcpy \n");
                    memcpy(buf,pMixbuf,gAACAudioEncode.nMixBufferLength-mixReadIdx);
                    pMixbuf = (short*) &gAACAudioEncode.pMixBuffer[0];
                    memcpy(&buf[gAACAudioEncode.nMixBufferLength-mixReadIdx],pMixbuf,nTemp-(gAACAudioEncode.nMixBufferLength-mixReadIdx));
                }
            }
        }
        else
        ...
        if (gAACAudioEncode.nSampleSize==16){
            ...
            {
                // volume control
                ControlVolume(pSrcbuf, gAACEncoder.nb_channels * 1024);
                for (i= 0;i<1024 *gAACEncoder.nb_channels;i++){
            ...
            {
                // volume control
                ControlVolume(pSrcbuf, (gAACAudioEncode.nBufferLength-wavReadIdx) >> 1);
                ControlVolume((short*) &gAACAudioEncode.pInputBuffer[0], (nTemp-(gAACAudioEncode.nBufferLength-wavReadIdx)) >> 1);
...
static int FillReadBuffer_I2S_Resample_Mixer(ResampleAudioContext *audioContext) 
{
    ...
    int nTmp;
    int nTemp,bytesLeft, nTempMixer;
    int i,j;
    short* buf;// = (char *)&pcmWriteBuf[pcmWriteIdx];
    short* pMixbuf;
    ...
    //nTemp = MPA_FRAME_SIZE*2*gMpegAudioContext.nb_channels*(gAACEncoder.nSampleSize/16);
    nTemp = audioContext->nInSize*(gAACAudioEncode.nSampleSize/16);
    nTempMixer = gAACEncoder.nb_channels*2*1024*(gAACAudioEncode.nSampleSize/16);
    ...
    if (gAACAudioEncode.nInputBufferType==2)
    {
        do
        {
            ...
            for (i = 0; i < 512; i++) asm("");
#endif
        }while (bytesLeft<=nTempMixer && !isSTOP());
        ...
        if (bNoMixer)
        {
            if (gAACAudioEncode.nSampleSize==16)
            {
                ...
                nTmp = resample(&avResampleContext,audioContext,(short*)inputBuf);
                // volume control
                ControlVolume((short*) inputBuf, gAACEncoder.nb_channels * 1024);
            }
        }
        else if (bNoI2S)
        {
            if (gAACAudioEncode.nSampleSize==16)
            {
                if (nTempMixer+mixReadIdx<=gAACAudioEncode.nMixBufferLength){
                    memcpy(inputBuf,pMixbuf,nTempMixer);
                } else {
                    //printf("[AAC Enc] memcpy \n");
                    memcpy(inputBuf,pMixbuf,gAACAudioEncode.nMixBufferLength-mixReadIdx);
                    pMixbuf = (short*) &gAACAudioEncode.pMixBuffer[0];
                    memcpy(&inputBuf[gAACAudioEncode.nMixBufferLength-mixReadIdx],pMixbuf,nTempMixer-(gAACAudioEncode.nMixBufferLength-mixReadIdx));
                }
            }
        }
        else
        {
            if (gAACAudioEncode.nSampleSize==16){
                ...
                // volume control
                ControlVolume((short*) inputBuf, gAACEncoder.nb_channels * 1024);
                //memset(inputBuf, 0x0, sizeof(short) * 1024 * gAACEncoder.nb_channels);
                buf = (short*) inputBuf;
                if (nTempMixer+mixReadIdx<=gAACAudioEncode.nMixBufferLength)
                {
                    for (i= 0;i<1024 * gAACEncoder.nb_channels;i++)
        ...
        if (nTempMixer > 0) {
            if (!bNoMixer)
            {
                mixReadIdx = mixReadIdx + nTempMixer;
                if (mixReadIdx >= gAACAudioEncode.nMixBufferLength) {
                    mixReadIdx -= gAACAudioEncode.nMixBufferLength;
                }
                setMixerReadPorinter(mixReadIdx);
...
// resample setting
        if (gAACAudioEncode.nSampleRate != gAACAudioEncode.nOutSampleRate) {            
            printf("[AAC Enc] 1 resampleInit, nSample: %u, nOut: %u \n", gAACAudioEncode.nSampleRate, gAACAudioEncode.nOutSampleRate);
            ...
            printf("[AAC Enc] 1 av_resample_init \n");            
            nNeedResample = 1;
        } else {
            nNeedResample = 0;
        }
// 
    ...
    while (1) {
        ...
        if (gAACAudioEncode.nSampleRate != gAACAudioEncode.nOutSampleRate) {
            if(nNeedResample == 0) {            
                printf("[AAC Enc] 2 resampleInit, nSample: %u, nOut: %u \n", gAACAudioEncode.nSampleRate, gAACAudioEncode.nOutSampleRate);
                resampleInit(&resampleAudioContext);
                av_resample_init(&avResampleContext,gAACAudioEncode.nOutSampleRate, gAACAudioEncode.nSampleRate, RESAMPLE_FILTER_LENGTH, 10,0, 0.8);
                printf("[AAC Enc] 2 av_resample_init \n");            
                nNeedResample = 1;
            }
        } else {
            if (nNeedResample) {
                printf("From resampling to not resample: %u\n", gAACAudioEncode.nSampleRate);
                nNeedResample = 0;
            }
        }
