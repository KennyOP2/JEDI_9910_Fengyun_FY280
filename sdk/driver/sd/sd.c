...
static MMP_INT SD_SwitchFunction11(SD_CTXT* ctxt)
{
    MMP_INT result = 0;
    MMP_UINT8 status[SD_SWITCH_FUN_LEN] = {0};
    MMP_INT i = 0;
    MMP_UINT32 tmp = 0;
    ...
    while(1)
    {
        ...
        {
            tmp = SD_ReadDataReg();
            *((MMP_UINT32*)&status[i]) = cpu_to_le32(tmp);
        }
...
{
    MMP_INT result = 0;
    SYS_CLOCK_T lastTime = 0, sleepTime = 0;
    ...
    do
    {
        if(SYS_GetDuration(lastTime) > 1300)
        {
...
SD_Switch(SD_CTXT* ctxt)
{
    ...
    ctxt->clockDiv = SD_CLK_NORMAL_INIT; // Default mode : 100/5 = 20 MHz
    ...
    tmp = SD_ReadDataReg();
    *((MMP_UINT32*)&scr[0]) = cpu_to_le32(tmp);
    tmp = SD_ReadDataReg();
    *((MMP_UINT32*)&scr[4]) = cpu_to_le32(tmp);
    result = SDW_WaitWrapReadyReg(ctxt);
    if(result)
        goto end;
    ...
    if(scr[0] & 0x0F)
    {
        ...
        if(SD_SwitchFunction11(ctxt))
        {
            ...
            ctxt->clockDiv = SD_CLK_NORMAL; // 100/5 = 20
            LOG_INFO " SD switch High Speed fail!! \n" LOG_END
        }
        else
        {
            /** Run with High Speed. */
            ctxt->clockDiv = SD_CLK_HIGH_SPEED; /** High Speed */ // 100/3 = 33.33
