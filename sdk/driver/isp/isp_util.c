    ...
    int         columns)
{
    ...
    if (((*array3D) = (MMP_FLOAT***)calloc(frames, sizeof(MMP_FLOAT**))) == MMP_NULL)
        isp_msg_ex(ISP_MSG_TYPE_ERR, " get_mem3Dfloat: array3D %s() !\n", __FUNCTION__);

    for (j = 0; j < frames; j++)
        get_mem2Dfloat( (*array3D)+j, rows, columns ) ;
    ...
    int         columns)
{
    ...
    if ((*array2D      = (MMP_FLOAT**)calloc(rows,        sizeof(MMP_FLOAT*))) == MMP_NULL)
        isp_msg_ex(ISP_MSG_TYPE_ERR, " get_mem2Dfloat: array2D %s() !\n", __FUNCTION__);
    ...
    int         frames)
{
    ...
    if (array3D)
    {
        for (i = 0; i < frames; i++)
        {
            free_mem2Dfloat(array3D[i]);
        }
        free(array3D);
    }
    else
    {
        isp_msg_ex(ISP_MSG_TYPE_ERR, " free_mem3Dfloat: trying to free unused memory %s()!\n", __FUNCTION__);
    }
}
    ...
    MMP_FLOAT   **array2D)
{
    if (array2D)
    {
        if (array2D[0])
            free(array2D[0]);
        else isp_msg_ex(ISP_MSG_TYPE_ERR, " free_mem2Dfloat: trying to free unused memory %s()!\n", __FUNCTION__);

        free(array2D);
    }
    else
    {
        isp_msg_ex(ISP_MSG_TYPE_ERR, " free_mem2Dfloat: trying to free unused memory %s()!\n", __FUNCTION__);
    }
}
