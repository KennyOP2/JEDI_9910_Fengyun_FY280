...
static MMP_UINT16 gtCAT9883CurMode  = 0;
static MMP_UINT16 gtCAT9883NextMode = 0;
...
MMP_UINT8 PC_1360x768x60[] =                     // Clive 041008
{
...
MMP_UINT8 PC_1280x800x60[] =                       // mode = 0xC8         Clive 0407     1680 pixel
{
...
MMP_UINT8 PC_1280x960x60[] =                       // mode = 0x73
{
...
void Load_Reg_ini(REGPAIR *pREGPAIR, MMP_UINT16 N)
{
    MMP_UINT8  i;
    MMP_UINT8  Value;
    MMP_RESULT result;
...
//kenny patch 20140428   disable macrovision
    {
        MMP_UINT8 val;
        val = CAT9883_ReadI2C_Byte(0x80);
        printf("CAT9883 0X80 pre= %x\n", val);
        val = val | 0x20;
        CAT9883_WriteI2C_Byte(0x8F, val);
        printf("CAT9883 0X80 post = %x\n", val);
    }
...
MMP_UINT16 TV_mode(MMP_BOOL isFrameRateCheck)
{
    ...
    else if (hsynccnt < 0x0120)
    {
        Resolution_mode = 0xa400;
    }                                                       // 480i = 0x107
    else if (hsynccnt < 0x0180)
    {
        Resolution_mode = 0xa500;
    }                                                       // 576i = 0x138
    else if (hsynccnt < 0x0220)
    {
        Resolution_mode = 0xb400;
    }                                                       // 480P = 0x20d
    else if (hsynccnt < 0x0250)
    {
        Resolution_mode = 0xa100;
    }                                                       // 1080i= 0x233
    else if (hsynccnt < 0x02a0)
    {
        Resolution_mode = 0xb500;
    }                                                       // 576P = 0x271
    else if (hsynccnt < 0x03a0)
    {
        Resolution_mode = 0xb700;
    }                                                       // 720P = 0x2ee
    else if (hsynccnt < 0x04a0)
    {
...
MMP_UINT16 PC_mode()
{
    ...
    if (Hsync_counter() < 0x0170)
    {
        Resolution_mode = 0xff00;
    }                                                                                            // <xxx unknow
    else if (Hsync_counter() >= 0x01a0 && Hsync_counter() < 0x0240)
    {
        Resolution_mode = 0x6400;
    }                                                                                            // 640x480 = 0x20d
    else if (Hsync_counter() >= 0x0240 && Hsync_counter() < 0x02d0)
    {
        Resolution_mode = 0x8000;
    }                                                                                            // 800x600 = 0x0274

    else if (Hsync_counter() >= 0x02d0 && Hsync_counter() < 0x0310)
    {
        Resolution_mode = 0x0300;
    }                                                                                            // 1280x720 = 0x02EE          //Clive

    else if (Hsync_counter() >= 0x0310 && Hsync_counter() < 0x031d)
    {
        Resolution_mode = 0x1300;
    }                                                                                            // 1360x768 = 0x031b
    else if (Hsync_counter() >= 0x031e && Hsync_counter() < 0x0331)
    {
        Resolution_mode = 0x1000;
    }                                                                                            // 1024x768 = 0x0326/(320@75)
    else if (Hsync_counter() >= 0x0332 && Hsync_counter() < 0x0352)
    {
        Resolution_mode = 0xC800;
    }                                                                                            // 1280x800  V_total=828=0x33c  Clive 0407
    else if (Hsync_counter() >= 0x0360 && Hsync_counter() < 0x039f)
    {
        Resolution_mode = 0x1480;
    }                                                                                            // 1440x900 reduced blanking
    else if (Hsync_counter() >= 0x03a0 && Hsync_counter() <= 0x03a7)
    {
        Resolution_mode = 0x1400;
    }                                                                                            // 1440x900
    else if (Hsync_counter() >= 0x03c0 && Hsync_counter() < 0x0400)
    {
        Resolution_mode = 0xC900;
    }                                                                                            // 1280x960 = 0x03e8
    else if (Hsync_counter() >= 0x0420 && Hsync_counter() < 0x0437)
    {
        Resolution_mode = 0x1200;
    }                                                                                            // 1280x1024 = 0x042a
    else if (Hsync_counter() >= 0x0437 && Hsync_counter() <= 0x043a)
    {
        Resolution_mode = 0x1680;
    }                                                                                            // 1680x1050 reduced blanking =
    else if (Hsync_counter() >= 0x0440 && Hsync_counter() < 0x0459)
    {
        Resolution_mode = 0x1600;
    }                                                                                            // 1680x1050
    else if (Hsync_counter() >= 0x0459 && Hsync_counter() < 0x0490)
    {
        Resolution_mode = 0x1900;
    }                                                                                            // 1920x1080 = 0x465  // Clive 041108 0x460 => 0x459
    else if (Hsync_counter() >= 0x04d0 && Hsync_counter() <= 0x04d6)
    {
        Resolution_mode = 0x1980;
    }                                                                                            // 1920x1200 reduced blanking = 0x04d3
    else if (Hsync_counter() >= 0x04da && Hsync_counter() < 0x0510)
    {
    ...
    if ((gtCAT9883CurMode & 0xFFFF) != (Resolution_mode + Frame_rate()))
    {
        ...
        if (Resolution_mode == 0x6400)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 640x480--------\n");
        else if (Resolution_mode == 0x8000)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 800x600--------\n");
        else if (Resolution_mode == 0x0300)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1280x720--------\n");
        else if (Resolution_mode == 0x1300)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1360x768--------\n");
        else if (Resolution_mode == 0x1000)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1024x768--------\n");
        else if (Resolution_mode == 0xC800)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1280x800--------\n");
        else if (Resolution_mode == 0x1480)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1440x900 reduced blanking--------\n");
        else if (Resolution_mode == 0x1400)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1440x900--------\n");
        else if (Resolution_mode == 0xC900)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1280x960--------\n");
        else if (Resolution_mode == 0x1200)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1280x1024--------\n");
        else if (Resolution_mode == 0x1680)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1680x1050 reduced blanking--------\n");
        else if (Resolution_mode == 0x1600)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1680x1050--------\n");
        else if (Resolution_mode == 0x1900)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1920x1080--------\n");
        else if (Resolution_mode == 0x1980)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1920x1200 reduced blanking--------\n");
        else if (Resolution_mode == 0x1C00)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 PC Mode Resolution = 1600x1200--------\n");

        if (Resolution_mode != 0xFF00 && Frame_rate() != 0xFF)
            dbg_msg(SDK_MSG_TYPE_INFO, "--------frame rate = %x fps--------\n", Frame_rate());
    }
...
MMP_UINT16 Frame_rate()
{
    ...
    static MMP_UINT16 preFrameRate_mode = 0xff;
...
// 1920x1080                        1960
// 1920x1200RB                                                              1980+60
void Set_mode(MMP_UINT16 Mode_number)
{
...
// 1080P                            b160
    case 0xa460:
        ...
        CAT9883_InFrameRate   = 2997;
        CAT9883_InIsInterlace = 1;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 480I60--------\n");
        break;
    ...
    case 0xa550:
        ...
        CAT9883_InIsInterlace = 1;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 576I50--------\n");
        break;
    ...
    case 0xb460:
        ...
        CAT9883_InFrameRate   = 5994;
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 480P60--------\n");
        break;
    ...
    case 0xa150:
        ...
        CAT9883_InFrameRate   = 2500;
        CAT9883_InIsInterlace = 1;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 1080I50--------\n");
        break;
    ...
    case 0xa160:
        ...
        CAT9883_InFrameRate   = 2997;
        CAT9883_InIsInterlace = 1;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 1080I60--------\n");
        break;
    ...
    case 0xb550:
        ...
        CAT9883_InFrameRate   = 5000;
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 576P50--------\n");
        break;
    ...
    case 0xb750:
        ...
        CAT9883_InFrameRate   = 5000;
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 720P50--------\n");
        break;
    ...
    case 0xb760:
        ...
        CAT9883_InFrameRate   = 5994;
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 720P60--------\n");
        break;
    ...
    case 0xb124:
        ...
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 1080P24--------\n");
        break;
    ...
    case 0xb150:
        ...
        CAT9883_InFrameRate   = 5000;
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 1080P50--------\n");
        break;
    ...
    case 0xb160:
        ...
        CAT9883_InFrameRate   = 5994;
        CAT9883_InIsInterlace = 0;
        dbg_msg(SDK_MSG_TYPE_INFO, "--------CAT9883 TV Mode Resolution = 1080P60--------\n");
        break;
