...
/*static*/ MMP_UINT16 gtHDMIResolution  = 0xFF; //kenny patch 20140428
...
{
    if (id == MMP_CAP_DEV_ADV7180_S)
    {
        gtIsSVideoOMode = MMP_TRUE;
        INPUT_DEVICE    = MMP_CAP_DEV_ADV7180;
    }
    else
    {
...
mmpCapGetCaptureDevice(
    void)
{
    //if (INPUT_DEVICE == MMP_CAP_DEV_ADV7180 && gtIsSVideoOMode)
    //	return MMP_CAP_DEV_ADV7180_S;
    //else
    return INPUT_DEVICE;
}
    ...
    void)
{
    ...
    if (gtDeviceReboot)
    {
        ...
        if (INPUT_DEVICE != MMP_CAP_DEV_ADV7180)
            ...
            mmpADV7180Initialize(MMP_FALSE);
        mmpADV7180OutputPinTriState(MMP_TRUE);
#endif
    ...
    if (INPUT_DEVICE == MMP_CAP_DEV_HDMIRX)
    {
        ...
        for (i = 0; i < (sizeof(HDMI_TABLE) / sizeof(CAP_HDMI_TIMINFO_TABLE)); ++i)
        {
                ...
                (VTotal == HDMI_TABLE[i].VTotal || VTotal == HDMI_TABLE[i].VTotal + 1) )
            {
                ...
                data->inputPAR       = mmpHDMIRXGetProperty(HDMIRX_ASPECT_RATIO);
...
mmpCapGetInputInfo(
    MMP_CAP_SHARE *data)
{
    memcpy(data, &DevInfo, sizeof(MMP_CAP_SHARE));
}
