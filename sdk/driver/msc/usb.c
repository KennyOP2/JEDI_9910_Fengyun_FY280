...
//=============================================================================
static void* msc_probe(struct usb_device *dev, MMP_UINT intf, const struct usb_device_id *id)
{
    const int id_index = id - storage_usb_ids;
    struct us_data* ss = MMP_NULL;
    ...
    LOG_INFO "Endpoints: Bulk-in: %d, Bulk-out: %d, Interrupt: 0x%p (Period %d)\n",
          ss->ep_in, ss->ep_out, ss->ep_int, (ss->ep_int ? ss->ep_int->bInterval : 0) LOG_END
...
static void msc_disconnect(struct usb_device* dev, void* ptr)
{
    struct us_data* ss = MMP_NULL;

    if(us_list[0].pusb_dev == dev)
        ss = &us_list[0];
    else if(us_list[1].pusb_dev == dev)
        ss = &us_list[1];
    else
        LOG_ERROR " msc_disconnect() no available context?? \n" LOG_END
...
{
    INIT_LIST_HEAD(&msc_driver.driver_list);
    msc_driver.serialize = SYS_CreateSemaphore(1, MMP_NULL);
