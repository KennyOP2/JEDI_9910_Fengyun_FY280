...
static ULONG    gtDefaultIEEEOUI;
static BYTE     gtDefaultDC;
#endif
...
// Program utility.
////////////////////////////////////////////////////////////////////////////////
#ifdef SUPPORT_AUTODETECT_DVI
BYTE ValidHDMIForParseEDID();
#endif
...
_IDATA BYTE bEDIDOutputColorMode = EDID_MODE_RGB444;
...
HDMITX_SetOutput(unsigned int AudioSampleRate, unsigned int AudioChannelNum, unsigned int inputDevice)
{
    ...
    inputColorMode = bInputColorMode & F_MODE_CLRMOD_MASK;

    if (gtInputDevice == TXIN_HDMIRX)
    {
        if (inputColorMode == F_MODE_YUV444 || inputColorMode == F_MODE_YUV422)
        {
            if (bHDMIMode)
            {
                if (bEDIDOutputColorMode & EDID_MODE_YUV444)
                    bOutputColorMode = F_MODE_YUV444;
                else if (bEDIDOutputColorMode & EDID_MODE_YUV422)
                    bOutputColorMode = F_MODE_YUV422;
                else if (bEDIDOutputColorMode & EDID_MODE_RGB444)
    ...
    else //YPbPr or CVBS or S-Video
    {
        ...
        {
            if(bEDIDOutputColorMode & EDID_MODE_RGB444)
                bOutputColorMode = F_MODE_RGB444;
            else if (bEDIDOutputColorMode & EDID_MODE_YUV444)
                bOutputColorMode = F_MODE_YUV444;
            else if (bEDIDOutputColorMode & EDID_MODE_YUV422)
                bOutputColorMode = F_MODE_YUV422;
        }
...
#else
            EnableHDMIAudio(T_AUDIO_LPCM, gtEnSPDIFIn, (ULONG)AudioSampleRate, (BYTE)AudioChannelNum, NULL, TMDSClock);
#endif
            ConfigAudioInfoFrm();
        }
    }
...
HDMITX_DevLoopProc(unsigned char bHDMIRxModeChange, unsigned int AudioSampleRate, unsigned int AudioChannelNum,
                    TXInput_Device inputDevice, unsigned int EnSPDIFIn)
{
    unsigned long TMDSClock = VideoPixelClock*(pixelrep+1);
    BYTE audioStatus;
    ...
    {
        BYTE changeSPDIF;
        static BYTE bAudioSetOK = FALSE;
        static BYTE bInitAudioSet = FALSE;

        if (gtEnSPDIFIn != EnSPDIFIn)
            changeSPDIF = TRUE;
        else
            changeSPDIF = FALSE;

        if (HPDStatus == TRUE && HPDChangeStatus == FALSE && bAudioSetOK == TRUE &&
            bHDMIRxModeChange == FALSE && changeSPDIF == FALSE)
        {
            if (HDMITX_Interrupt())
                goto lab_checkHDMITX;
        }
        else
        {
lab_checkHDMITX:
            CheckHDMITX(&HPDStatus, &HPDChangeStatus, &audioStatus);

            if ((audioStatus == 1 && EnSPDIFIn == 1) || (audioStatus == 2 && EnSPDIFIn == 0))
            {
                bAudioSetOK = FALSE;
                bInitAudioSet = TRUE;
            }
            else if (audioStatus == 0 && bInitAudioSet)
            {
                 bAudioSetOK = TRUE;
                 bInitAudioSet = FALSE;
            }
    ...
    if( HPDChangeStatus || bHDMIRxModeChange)
    {
        if( HPDStatus )
        {
...
#else
#ifdef SUPPORT_AUTODETECT_DVI
            if (!ValidHDMIForParseEDID())
                printf("---ValidHDMIForParseEDID read edid fail---\n");
#endif
#endif
            bEDIDOutputColorMode = EDID_MODE_RGB444;
            ...
            if( RxCapability.ValidHDMI )
            {
                ...
                {
                    bEDIDOutputColorMode |= EDID_MODE_YUV444;
                }

                if (RxCapability.VideoMode & (1<<4))
                {
                    bEDIDOutputColorMode |= F_MODE_YUV422;
                }
            }
            else
            {
    ...
    else // no stable but need to process mode change procedure
    {
        ...
        if (bAudioEnable)
        {
            if ((preAudioNum != AudioChannelNum) || (preAudioRate != AudioSampleRate) || EnSPDIFIn != gtEnSPDIFIn)
            {
                DisableAudioOutput();
#ifdef SUPPORT_HBR_AUDIO
                EnableHDMIAudio(T_AUDIO_HBR, FALSE, 768000L,OUTPUT_CHANNEL,NULL, TMDSClock);
...
BYTE ValidHDMIForParseEDID()
{
    BYTE ParseState = TRUE;
    BYTE BlockCount = 0;
    BYTE CEAEDID[128] = {0};
    BYTE *pCEAEDID = CEAEDID;
    BYTE IEEEOUI[3] = {0};
    BYTE isHDMIMode = FALSE;

    if (ReadEDID(&BlockCount, 0, 0x7E, 1) == ER_FAIL)
        return FALSE;
    ...
    if (BlockCount == 0)
    {
        ...
        RxCapability.IEEEOUI = 0x0;
    }
    else
    {
        RxCapability.ValidCEA = TRUE;
        RxCapability.ValidHDMI = TRUE;
        RxCapability.IEEEOUI = gtDefaultIEEEOUI;
        RxCapability.dc.uc = gtDefaultDC;
    }
...
ParseCEAEDID(BYTE *pCEAEDID)
{
    ...
    for( offset = 4 ; offset < End ; )
    {
        ...
        case 0x03: // Vendor Specific Data Block ;
            ...
            gtDefaultIEEEOUI = RxCapability.IEEEOUI;
            gtDefaultDC = RxCapability.dc.uc;
...
BYTE HDMITX_Interrupt(void)
{
    if (GPIO_GetState(30) == GPIO_STATE_HI)
        return FALSE;
    else
        return TRUE;
}
