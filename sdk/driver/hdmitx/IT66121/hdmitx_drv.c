...
void hdmitx_LoadRegSetting(RegSetEntry table[])
{
    ...
    for( i = 0 ;  ; i++ )
    {
        ...
        {
            //HDMITX_DEBUG_PRINTF(("HDMITX_WriteI2C_Byte(%02x,%02x)\n",(int)table[i].offset,(int)table[i].OrMask));
...
EnableHDCP(BYTE bEnable)
{
    ...
    if(bEnable)
    {
        ...
        {
			HDMITX_DEBUG_PRINTF(("EnableHDCP: HDCP_ResetAuth()\n") );
            HDCP_ResetAuth();
            return FALSE ;
        }
...
{
	if(B_RXSENDETECT & HDMITX_ReadI2C_Byte(REG_TX_SYS_STATUS))
    {
        if(0==HDMITX_ReadI2C_Byte(REG_TX_AFE_DRV_CTRL))
        {
            //HDMITX_DEBUG_PRINTF(("getHDMITX_LinkStatus(): return TRUE!!\n") );
            return TRUE;
        }
    }
...
CheckHDMITX(BYTE *pHPD,BYTE *pHPDChange)
{
    ...
    BYTE HPD ;
	//HDMITX_DEBUG_PRINTF(("CheckHDMITX: Enter, HPD=%d, HPDChange=%d\r\n", *pHPD, *pHPDChange));
    ...
    Instance[0].bHPD = HPD ;
	//HDMITX_DEBUG_PRINTF(("CheckHDMITX: Exit, HPD=%d, HPDChange=%d\r\n", *pHPD, *pHPDChange));
    return HPD ;
}
...
DisableAudioOutput()
{
    //BYTE uc = (HDMITX_ReadI2C_Byte(REG_TX_SW_RST) | (B_AUD_RST_HDMITX | B_AREF_RST)) ;
    //HDMITX_WriteI2C_Byte(REG_TX_SW_RST,uc);
    HDMITX_OrReg_Byte(REG_TX_SW_RST,(B_AUD_RST_HDMITX | B_AREF_RST));
}
...
typedef struct structTxSyncDeGenEntry {
     MODE_ID CaseNo;
    BYTE CEAFmt;
    BYTE RegValues[23];
}TxSyncDeGenEntry;

static _CODE TxSyncDeGenEntry s_txSyncDeGenTable[] =
{
    // case 0x0150: //720x576(P)@50Hz (Fmt17)
  //  {CEA_720x576p50, 17, {0xf9, 0x35, 0x84, 0x54, 0x30, 0x00, 0x40, 0x00, 0x00, 0x00, 0x2a, 0x6a, 0x20, 0xff, 0xff, 0xff, 0x70, 0x32, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    // case 0x0260: //720x480(P)@60Hz (Fmt2)
   // {CEA_720x480p60, 2, {0x99, 0x35, 0x78, 0x48, 0x30, 0x58, 0x3c, 0x03, 0x00, 0x00, 0x23, 0x03, 0x20, 0xff, 0xff, 0xff, 0x0c, 0x02, 0xff, 0x5f, 0xf0, 0x00, 0x00, }},
    // case 0x0360: //1280x720(P)@60Hz (Fmt4)
   // {CEA_1280x720p60, 4, {0x7f, 0x16, 0x06, 0x06, 0x61, 0x02, 0x2a, 0x00, 0xd0, 0x02, 0x18, 0xe8, 0x20, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    // case 0x0450: //1920x1080(I)@50Hz (Fmt20)
   // {CEA_1920x1080i50, 20, {0xaf, 0x52, 0xc2, 0x42, 0x80, 0x02, 0x2e, 0x00, 0x60, 0x04, 0x13, 0x2f, 0x20, 0x46, 0x62, 0x42, 0x00, 0x50, 0x32, 0x72, 0x30, 0x00, 0x00, }},
    // case 0x1960: //PC mode 1920x1080(P)@60Hz (Fmt16)
   // {CEA_1920x1080p60, 16, {0x9f, 0x78, 0xc2, 0x42, 0x80, 0x02, 0x2e, 0x00, 0x30, 0x04, 0x28, 0x60, 0x40, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x4f, 0xf0, 0x00, 0x00, }},
    // case 0x1950: //1920x1080(P)@50Hz
   // {CEA_1920x1080p50, 31, {0x8f, 0x97, 0xc2, 0x42, 0x80, 0x02, 0x2e, 0x00, 0x30, 0x04, 0x28, 0x60, 0x40, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    // case 0xa460: //720x480(I)@60Hz (Fmt6)
    {CEA_720x480i60, 6, {0xc9, 0x1a, 0x76, 0x46, 0x30, 0x59, 0x3d, 0x03, 0x00, 0x00, 0x10, 0x00, 0x10, 0x17, 0x07, 0x21, 0x0b, 0x12, 0x05, 0x81, 0xf0, 0x00, 0x00, }},
    // case 0xa550: //720x576(I)@50Hz (Fmt21)
    {CEA_720x576i50, 21, {0x09, 0x1b, 0x84, 0x54, 0x30, 0x00, 0x3f, 0x00, 0x00, 0x00, 0x14, 0x34, 0x10, 0x4d, 0x6d, 0x21, 0x70, 0x22, 0x37, 0xa1, 0xf0, 0x00, 0x00, }},
    // case 0xb460: //720x480(P)@60Hz (Fmt2)
  {CEA_720x480p60, 2, {0x99, 0x35, 0x78, 0x48, 0x30, 0x58, 0x3c, 0x03, 0x00, 0x00, 0x23, 0x03, 0x20, 0xff, 0xff, 0xff, 0x0c, 0x52, 0xff, 0x5f, 0xf0, 0x00, 0x00, }},
    // case 0xa150: //1920x1080(I)@50Hz (Fmt20)
    {CEA_1920x1080i50, 20, {0xaf, 0x52, 0xe2, 0x62, 0x80, 0x02, 0x2e, 0x00, 0x60, 0x04, 0x13, 0x2f, 0x20, 0x46, 0x62, 0x42, 0x00, 0x50, 0x32, 0x72, 0x30, 0x00, 0x00, }},
    // case 0xa160: //1920x1080(I)@60Hz (Fmt5)
    {CEA_1920x1080i60, 5, {0xef, 0x44, 0xe2, 0x62, 0x80, 0x02, 0x2e, 0x00, 0x60, 0x04, 0x13, 0x2f, 0x20, 0x46, 0x62, 0x42, 0x00, 0x50, 0x32, 0x72, 0x30, 0x00, 0x00, }},
    // case 0xb550: //720x576(P)@50Hz (Fmt17)
    {CEA_720x576p50, 17, {0xf9, 0x35, 0x84, 0x54, 0x30, 0x00, 0x40, 0x00, 0x00, 0x00, 0x2a, 0x6a, 0x20, 0xff, 0xff, 0xff, 0x70, 0x42, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    // case 0xb750: //1280x720(P)@50Hz (Fmt19)
    {CEA_1280x720p50, 19, {0x7f, 0x16, 0x36, 0x36, 0x61, 0x02, 0x2a, 0x00, 0xd0, 0x02, 0x18, 0xe8, 0x20, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x4f, 0xf0, 0x00, 0x00, }},
    // case 0xb760: //1280x720(P)@60Hz (Fmt4)
    {CEA_1280x720p60, 4, {0x7f, 0x16, 0x36, 0x36, 0x61, 0x02, 0x2a, 0x00, 0xd0, 0x02, 0x18, 0xe8, 0x20, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    // case 0xb160: //1920x1080(P)@60Hz (Fmt16)
    //{0xb160, 16, {0x9f, 0x78, 0xc2, 0x42, 0x80, 0x02, 0x2e, 0x00, 0x30, 0x04, 0x28, 0x60, 0x40, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    {CEA_1920x1080p60, 16, {0xff, 0xff, 0xe0, 0x60, 0x80, 0x00, 0x2c, 0x00, 0x60, 0x04, 0x2a, 0x62, 0x40, 0xff, 0xff, 0x00, 0x01, 0x60, 0xff, 0xf0, 0xf0, 0x00, 0x00, }},
    // case 0xb150: //1920x1080(P)@50Hz
    //{0xb150, 31, {0x8f, 0x97, 0xc2, 0x42, 0x80, 0x02, 0x2e, 0x00, 0x30, 0x04, 0x28, 0x60, 0x40, 0xff, 0xff, 0xff, 0x00, 0x50, 0xff, 0x3f, 0xf0, 0x00, 0x00, }},
    {CEA_1920x1080p50, 31, {0x9f, 0x78, 0xda, 0x5a, 0x80, 0x20, 0x4c, 0x00, 0x30, 0x04, 0x27, 0x5f, 0x40, 0xff, 0xff, 0xff, 0x64, 0x34, 0xff, 0x3f, 0x00, 0x00,0x00, }},
   // {VESA_GET_HDMIRX_DE, 31, {0x7f, 0x89, 0xe0, 0x60, 0x80, 0x20, 0x4c, 0x00, 0x30, 0x04, 0x27, 0x5f, 0x40, 0xff, 0xff, 0xff, 0x64, 0x34, 0xff, 0x3f, 0xf0, 0x00,0x00, }},
};

static _CODE TxSyncDeGenEntry s_txSyncDeGenTablecvbs[] =
{
    {CEA_720x480i60, 6, {0xc9, 0x1a, 0x86, 0x56, 0x30, 0x59, 0x3d, 0x03, 0x00, 0x00, 0x10, 0x00, 0x10, 0x17, 0x07, 0x21, 0x0b, 0x12, 0x05, 0x81, 0xf0, 0x00, 0x00, }},
    {CEA_720x576i50, 21, {0x09, 0x1b, 0x84, 0x54, 0x30, 0x00, 0x3f, 0x00, 0x00, 0x00, 0x14, 0x34, 0x10, 0x4d, 0x6d, 0x21, 0x70, 0x22, 0x37, 0xa1, 0xf0, 0x00, 0x00, }},
};
void SetTxDEGen(MODE_ID id)
{
	BYTE TxCEAFmt = 0x0;
    BYTE caseFound = 0;
    BYTE i,off;

    printf("SetTxDEGen - mode %d\n",id);

    for (i=0 ; i<sizeof(s_txSyncDeGenTable)/sizeof(s_txSyncDeGenTable[0]) ; i++)
    {
        if ( s_txSyncDeGenTable[i].CaseNo == id )
        {
            for (off=0x90 ; off<=0xA3 ; off++)
            {
                HDMITX_WriteI2C_Byte( off, s_txSyncDeGenTable[i].RegValues[off-0x90] );
            }

            off = 0xA6;
            HDMITX_WriteI2C_Byte( off, s_txSyncDeGenTable[i].RegValues[off-0x90] );

            for (off=0xB1 ; off<=0xB2 ; off++)
            {
                HDMITX_WriteI2C_Byte( off, s_txSyncDeGenTable[i].RegValues[off-0x90] );
            }

            HDMITX_WriteI2C_Byte(0xd1, 0x28);
			
          printf(" id =%d,ceafmt=%d\r\n",id, s_txSyncDeGenTable[i].CEAFmt);
    ...
    if ( caseFound == 0 )
    {
        HDMITX_DEBUG_PRINTF((" ERR ***** Unsupport mode\n"));
    }
}

void SetTxDEGenCVBS(MODE_ID id)
{
	BYTE TxCEAFmt = 0x0;
    BYTE caseFound = 0;
    BYTE i,off;
    printf("SetTxDEGenCVBS - mode %d\n",id);
    for (i=0 ; i<sizeof(s_txSyncDeGenTablecvbs)/sizeof(s_txSyncDeGenTablecvbs[0]) ; i++)
    {
        if ( s_txSyncDeGenTablecvbs[i].CaseNo == id )
        {
            for (off=0x90 ; off<=0xA3 ; off++)
            {
                HDMITX_WriteI2C_Byte( off, s_txSyncDeGenTablecvbs[i].RegValues[off-0x90] );
            }
            off = 0xA6;
            HDMITX_WriteI2C_Byte( off, s_txSyncDeGenTablecvbs[i].RegValues[off-0x90] );
            for (off=0xB1 ; off<=0xB2 ; off++)
            ...
            HDMITX_WriteI2C_Byte(0xd1, 0x28);
          printf(" id =%d,ceafmt=%d\r\n",id, s_txSyncDeGenTablecvbs[i].CEAFmt);
            caseFound=1;
            break;
        }
    }
...
BOOL ProgramDEGenModeByID(MODE_ID id,BYTE bInputSignalType, TXInput_Device inputDevice)
{
    int i ;
        ...
        switch (inputDevice)
        {
            case TXIN_HDMIRX:
                if (id == VESA_GET_HDMIRX_DE)
                {
                    ...
                    Reg92 = (timingDE.HDES & 0xFF);
                    Reg93 = (timingDE.HDEE & 0xFF);
                    Reg94 = (((timingDE.HDEE >> 8) << 4) | (timingDE.HDES >> 8));

                    Reg9A = (timingDE.VDES & 0xFF);
                    Reg9B = (timingDE.VDEE & 0xFF);
                    Reg9C = (((timingDE.VDEE >> 8) << 4) | (timingDE.VDES >> 8));
            ...
            case TXIN_CVBS:
               ...
               #if 1
		      SetTxDEGenCVBS(id);
                HDMITX_SetI2C_Byte(REG_TX_SW_RST, 0x08, 0x00);	
		 #else	
                ...
                HDMITX_WriteI2C_Byte(0x90, CVBS_DeGen_Table[i].Reg90);
                HDMITX_WriteI2C_Byte(0x91, CVBS_DeGen_Table[i].Reg91);
                HDMITX_WriteI2C_Byte(0x92, CVBS_DeGen_Table[i].Reg92);
                HDMITX_WriteI2C_Byte(0x93, CVBS_DeGen_Table[i].Reg93);
                HDMITX_WriteI2C_Byte(0x94, CVBS_DeGen_Table[i].Reg94);
                HDMITX_WriteI2C_Byte(0x95, CVBS_DeGen_Table[i].Reg95);
                HDMITX_WriteI2C_Byte(0x96, CVBS_DeGen_Table[i].Reg96);
                HDMITX_WriteI2C_Byte(0x97, CVBS_DeGen_Table[i].Reg97);
                HDMITX_WriteI2C_Byte(0x9A, CVBS_DeGen_Table[i].Reg9A);
                HDMITX_WriteI2C_Byte(0x9B, CVBS_DeGen_Table[i].Reg9B);
                HDMITX_WriteI2C_Byte(0x9C, CVBS_DeGen_Table[i].Reg9C);
                HDMITX_WriteI2C_Byte(0x9D, CVBS_DeGen_Table[i].Reg9D);
                HDMITX_WriteI2C_Byte(0x9E, CVBS_DeGen_Table[i].Reg9E);
                HDMITX_WriteI2C_Byte(0x9F, CVBS_DeGen_Table[i].Reg9F);
                HDMITX_WriteI2C_Byte(0xA0, CVBS_DeGen_Table[i].RegA0);
                HDMITX_WriteI2C_Byte(0xA1, CVBS_DeGen_Table[i].RegA1);
                HDMITX_WriteI2C_Byte(0xA2, CVBS_DeGen_Table[i].RegA2);
                HDMITX_WriteI2C_Byte(0xA3, CVBS_DeGen_Table[i].RegA3); 
		#endif
                return TRUE;                    
            ...
            case TXIN_YPBPR:                               
                ...
                HDMITX_WriteI2C_Byte(0x90, YPBPR_DeGen_Table[i].Reg90);
                HDMITX_WriteI2C_Byte(0x91, YPBPR_DeGen_Table[i].Reg91);
                HDMITX_WriteI2C_Byte(0x92, YPBPR_DeGen_Table[i].Reg92);
                HDMITX_WriteI2C_Byte(0x93, YPBPR_DeGen_Table[i].Reg93);
                HDMITX_WriteI2C_Byte(0x94, YPBPR_DeGen_Table[i].Reg94);
                HDMITX_WriteI2C_Byte(0x95, YPBPR_DeGen_Table[i].Reg95);
                HDMITX_WriteI2C_Byte(0x96, YPBPR_DeGen_Table[i].Reg96);
                HDMITX_WriteI2C_Byte(0x97, YPBPR_DeGen_Table[i].Reg97);
                HDMITX_WriteI2C_Byte(0x9A, YPBPR_DeGen_Table[i].Reg9A);
                HDMITX_WriteI2C_Byte(0x9B, YPBPR_DeGen_Table[i].Reg9B);
                HDMITX_WriteI2C_Byte(0x9C, YPBPR_DeGen_Table[i].Reg9C);
                HDMITX_WriteI2C_Byte(0x9D, YPBPR_DeGen_Table[i].Reg9D);
                HDMITX_WriteI2C_Byte(0x9E, YPBPR_DeGen_Table[i].Reg9E);
                HDMITX_WriteI2C_Byte(0x9F, YPBPR_DeGen_Table[i].Reg9F);
                HDMITX_WriteI2C_Byte(0xA0, YPBPR_DeGen_Table[i].RegA0);
                HDMITX_WriteI2C_Byte(0xA1, YPBPR_DeGen_Table[i].RegA1);
                HDMITX_WriteI2C_Byte(0xA2, YPBPR_DeGen_Table[i].RegA2);
                HDMITX_WriteI2C_Byte(0xA3, YPBPR_DeGen_Table[i].RegA3);     
		#endif
...
BOOL
ProgramSyncEmbeddedByVIC(BYTE VIC,BYTE bInputSignalType)
{
    int i ;
    // if Embedded Video,need to generate timing with pattern register

    HDMITX_DEBUG_PRINTF(("ProgramSyncEmbeddedByVIC(%d,%x)\n",(int)VIC,(int)bInputSignalType));
...
#endif
BOOL
ProgramSyncEmbeddedByTiming(VIDEO_Timing *pVTiming,BYTE bInputSignalType)
{
    unsigned int temp, t2 ;
    BYTE uc ;
    // if Embedded Video,need to generate timing with pattern register

    HDMITX_DEBUG_PRINTF(("ProgramSyncEmbeddedByTiming(%dx%d(%dx%d),%x)\n",
    ...
    if( bInputSignalType & T_MODE_SYNCEMB )
    {
        ...
        if( pVTiming->ScanMode == INTERLACE )
        {
            ...
            HDMITX_WriteI2C_Byte(REG_TX_HVPol,uc); // Reg90
            temp >>= 4 ;
            uc = (BYTE)(temp);
            HDMITX_WriteI2C_Byte(REG_TX_HfPixel,uc); // Reg91
        }
        else
        {
            uc = 0xF0 ;
            ...
            HDMITX_WriteI2C_Byte(REG_TX_HfPixel,0xFF); // Reg91
        }
        ...
        uc = (BYTE)temp & 0xFF ;
        HDMITX_WriteI2C_Byte(REG_TX_HSSL,uc); // Reg95
        uc = (BYTE)t2 & 0xFF ;
        HDMITX_WriteI2C_Byte(REG_TX_HSEL,uc); // Reg96

        temp >>= 8 ; temp &= 0xF ;
        t2 >>= 4 ; t2 &= 0xF0 ;
        temp |= t2 ;
        uc =  (BYTE)(temp);
        HDMITX_WriteI2C_Byte(REG_TX_HSH,uc); // Reg97

        temp = pVTiming->VFrontPorch ;
        t2  = temp + pVTiming->VSyncWidth ;

        uc = (BYTE)temp & 0xFF ;
        HDMITX_WriteI2C_Byte(REG_TX_VSS1,uc); // RegA0

        uc = (BYTE)((t2&0xF)<<4);
        uc |= (BYTE)(temp >> 8) ;

        HDMITX_WriteI2C_Byte(REG_TX_VSE1,uc); // RegA1
        ...
        if( pVTiming->ScanMode == INTERLACE )
        {
            ...
            uc = (BYTE)temp&0xFF ;
            HDMITX_WriteI2C_Byte(REG_TX_VSS2,uc); // RegA2

            temp >>= 8 ; temp &= 0xF ;
            t2 &= 0xF ; t2 <<= 4 ;
            temp |= t2 ;
            uc = (BYTE)temp ;
            HDMITX_WriteI2C_Byte(REG_TX_VSE2,uc); // RegA3
        }
        else
        {
            HDMITX_WriteI2C_Byte(REG_TX_VSS2,0xFF); // RegA2
            HDMITX_WriteI2C_Byte(REG_TX_VSE2,0xFF); // RegA3
        }
        HDMITX_DEBUG_PRINTF(("REG = {0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X}\n",
            (int)HDMITX_ReadI2C_Byte(REG_TX_HVPol) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_HfPixel) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_HSSL) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_HSEL) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_HSH) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_VSS1) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_VSE1) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_VSS2) ,
            (int)HDMITX_ReadI2C_Byte(REG_TX_VSE2) )) ;
...
SetInputMode(BYTE InputMode,BYTE bInputSignalType)
{
    ...
    if(bInputSignalType & T_MODE_CCIR656)
    {
        ucData |= (B_2X656CLK| B_SYNCEMB/*kenny*/)  ; HDMITX_DEBUG_PRINTF(("CCIR656 mode\n"));
    }
...
SetCSCScale(BYTE bInputMode,BYTE bOutputMode)
{
    ...
    if(csc == B_HDMITX_CSC_RGB2YUV)
    {
        ...
        case F_MODE_ITU709|F_MODE_16_235:
            ...
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_16_235,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_RGB2YUV_ITU709_16_235,SIZEOF_CSCMTX);
            break ;
        case F_MODE_ITU709|F_MODE_0_255:
            HDMITX_DEBUG_PRINTF(("ITU709 0-255 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_0_255,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_RGB2YUV_ITU709_0_255,SIZEOF_CSCMTX);
            break ;
        case F_MODE_ITU601|F_MODE_16_235:
            HDMITX_DEBUG_PRINTF(("ITU601 16-235 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_16_235,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_RGB2YUV_ITU601_16_235,SIZEOF_CSCMTX);
            break ;
        case F_MODE_ITU601|F_MODE_0_255:
        default:
            HDMITX_DEBUG_PRINTF(("ITU601 0-255 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_0_255,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_RGB2YUV_ITU601_0_255,SIZEOF_CSCMTX);
            break ;
        }
    ...
    if (csc == B_HDMITX_CSC_YUV2RGB)
    {
        ...
        case F_MODE_ITU709|F_MODE_16_235:
            HDMITX_DEBUG_PRINTF(("ITU709 16-235 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_16_235,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_YUV2RGB_ITU709_16_235,SIZEOF_CSCMTX);
            break ;
        case F_MODE_ITU709|F_MODE_0_255:
            HDMITX_DEBUG_PRINTF(("ITU709 0-255 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_0_255,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_YUV2RGB_ITU709_0_255,SIZEOF_CSCMTX);
            break ;
        case F_MODE_ITU601|F_MODE_16_235:
            HDMITX_DEBUG_PRINTF(("ITU601 16-235 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_16_235,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_YUV2RGB_ITU601_16_235,SIZEOF_CSCMTX);
            break ;
        case F_MODE_ITU601|F_MODE_0_255:
        default:
            HDMITX_DEBUG_PRINTF(("ITU601 0-255 "));
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_YOFF,bTxCSCOffset_0_255,SIZEOF_CSCOFFSET);
            HDMITX_WriteI2C_ByteN(REG_TX_CSC_MTX11_L,bTxCSCMtx_YUV2RGB_ITU601_0_255,SIZEOF_CSCMTX);
            break ;
        }
    }
    #endif
...
//    HDMITX_DEBUG_PRINTF(("BKSV %02X %02X %02X %02X %02X\n",(int)BKSV[0],(int)BKSV[1],(int)BKSV[2],(int)BKSV[3],(int)BKSV[4]));
    ...
    if((BCaps & B_CAP_HDMI_REPEATER) == 0)
    {
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut --)
        {
            ...
            {
                HDMITX_DEBUG_PRINTF(("HDCP_Authenticate(): Authenticate done\n"));
                Instance[0].bAuthenticated = TRUE ;
                break ;
            }
...
void
setHDMITX_UpdateChStatFs(ULONG Fs)
{
    BYTE uc ;
...
void
setHDMITX_LPCMAudio(BYTE AudioSrcNum, BYTE AudSWL, BOOL bSPDIF)
{
    ...
    Switch_HDMITX_Bank(0);
    //HDMITX_WriteI2C_Byte(REG_TX_AUDIO_CTRL0,AudioEnable&0xF0);//2015.2.6, my.wei mask
    HDMITX_OrREG_Byte(REG_TX_AUDIO_CTRL0, AudioEnable);//2015.2.6, my.wei enable audio source
...
void
setHDMITX_NLPCMAudio() // no Source Num, no I2S.
{
    BYTE AudioEnable, AudioFormat ;
    BYTE i ;
...
void
setHDMITX_HBRAudio(BOOL bSPDIF)
{
    BYTE rst,uc ;
    Switch_HDMITX_Bank(0);
...
EnableHDMIAudio(BYTE AudioType, BOOL bSPDIF,  ULONG SampleFreq,  BYTE ChNum, BYTE *pIEC60958ChStat, ULONG TMDSClock)
{
    ...
    case T_AUDIO_LPCM:
        ...
        setHDMITX_ChStat(pIEC60958ChStat);
        setHDMITX_LPCMAudio((ChNum+1)/2, /*24*/SUPPORT_AUDI_AudSWL, bSPDIF);
        // can add auto adjust
        break ;
    }
