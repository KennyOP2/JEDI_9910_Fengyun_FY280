...
/** 6M channel table */
static Dword channel_table_6M[11] = {
	0, 45000, 51000, 57000, 63000, 69000, 79000, 85000, 93000, 99000, 105000
};

/** 7M channel table */
static Dword channel_table_7M[11] = {
	0, 45500, 52500, 59500, 66500, 90500, 97500, 104500, 114500, 124500, 128500
};

/** 8M channel table */
static Dword channel_table_8M[11] = {
	0, 42000, 50000, 58000, 66000, 74000, 82000, 114000, 122000, 130000, 138000
};
...
Dword IT9507_setFrequency (
	...
	if(IQ_fixed_table0[TABLE_NROW-1].frequency<frequency || IQ_fixed_table0[0].frequency>frequency ){
...
Dword IT9507_initialize (
	...
	modulator->ptrIQtableEx =  IQ_fixed_table0;
	modulator->busId = busId; 
	modulator->i2cAddr = i2cAddr;

	if (modulator->busId == (Byte)0xFFFF) {
		goto exit;
	}
	...
	modulator->c1DefaultValue = c1_default_value[1]<<8 | c1_default_value[0];
	modulator->c2DefaultValue = c2_default_value[1]<<8 | c2_default_value[0];
	modulator->c3DefaultValue = c3_default_value[1]<<8 | c3_default_value[0];
...
static Word     gtbandwidth = 0;
static Dword    gtfrequency = 0;
...
Dword IT9507_setTxModeEnable (
	...
	if(enable){
		...
		//mylin add, if gpio8 enable and Hi, then writeRegister(0xDDAB, 0) error ??????    	       	    
    	error = EagleUser_acquireChannel(modulator, gtbandwidth, gtfrequency);   
    	if (error) goto exit;        	
...
Dword IT9507_acquireTxChannel (
    ...
    gtbandwidth	= bandwidth;
    gtfrequency = frequency;
    
    //mylin mark if gpio8 enable and Hi, then writeRegister(0xDDAB, 0) error ??????
...
Dword IT9507_adjustOutputGain (
	...
	c1value_default = modulator->c1DefaultValue;
	c2value_default = modulator->c2DefaultValue;
	c3value_default = modulator->c3DefaultValue;	
...
Dword IT9507_setIQtable (
	...
	if(IQ_table_ptr == NULL)
		...
		modulator->ptrIQtableEx = IQ_table_ptr;
