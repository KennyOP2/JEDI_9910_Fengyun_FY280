...
int IQ_table [TABLE_NROW][TABLE_NCOL];
	...
	)
{
  // Using binary search to find the frequency interval in the table
    ...
    int outdPhi;
	int diff;
    while( ! ( (fIn - IQ_table[idx][0]) >= 0 && (fIn - IQ_table[idx+1][0] < 0) ) ){
	  if((fIn - IQ_table[idx][0])==0)
		  break;
      preIdx = idx;
      if(fIn - IQ_table[idx+1][0] >= 0){
        idx = (preIdx + upper)/2;
        lower = preIdx;
      }else if(fIn - IQ_table[idx][0] < 0){
        idx = (preIdx + lower)/2;
        upper = preIdx;
      }
  ...
  // Perform linear interpolation
    outdAmp = IQ_table[idx][1] + (fIn - IQ_table[idx][0]) * (IQ_table[idx+1][1] - IQ_table[idx][1]) / (IQ_table[idx+1][0] - IQ_table[idx][0]);
    diff = (IQ_table[idx+1][2] - IQ_table[idx][2]);
	if(diff <= -8192) {
		diff = diff+16384;
	} else if(diff >= 8192) {
		diff = diff-16384;
	...
	outdPhi = IQ_table[idx][2] + (fIn - IQ_table[idx][0]) * diff / (IQ_table[idx+1][0] - IQ_table[idx][0]);
	if (outdPhi>=16384) {
...
Dword EagleTuner_setIQCalibration(
	...
	int test;
	int test2;
	for(row = 0; row < TABLE_NROW; row++){		
		IQ_table[row][0] = modulator->ptrIQtableEx[row].frequency;
		IQ_table[row][1] = modulator->ptrIQtableEx[row].dAmp;
		IQ_table[row][2] = modulator->ptrIQtableEx[row].dPhi;
	}

    interpolation(frequency, ptrdAmp, ptrdPhi);
...
//	int row = 0;
	...
	// Perform linear interpolation
	freqdiff = (int)(upperfreq.frequency - lowerfreq.frequency);
	fIndiff = (int)(fIn - lowerfreq.frequency);
	Ampdiff = (upperfreq.dAmp - lowerfreq.dAmp);
    dAmp = lowerfreq.dAmp + ((fIndiff * Ampdiff) / freqdiff);
    diff = (upperfreq.dPhi - lowerfreq.dPhi);
	if(diff <= -8192) {
		diff = diff+16384;
	} else if(diff >= 8192) {
		diff = diff-16384;
    ...
    //outdPhi = IQ_table[idx][2] + (fIn - IQ_table[idx][0]) * (IQ_table[idx+1][2] - IQ_table[idx][2]) / (IQ_table[idx+1][0] - IQ_table[idx][0]);
	dPhi = lowerfreq.dPhi + (fIndiff * diff) / freqdiff;
	if (dPhi>=16384) {
	dPhi = dPhi%16384;
   }
	while (dPhi<0) {
