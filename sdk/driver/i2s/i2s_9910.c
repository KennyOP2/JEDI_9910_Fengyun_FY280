...
static int _dev_running    = 0;
static int gtIsUseHdmiRx   = 0;
static int gtIsSlaveMode   = 0;
...
void i2s_deinit_ADC(MMP_BOOL bResetDevice);
void i2s_deinit_DAC(void);
void i2s_init_DAC(STRC_I2S_SPEC *i2s_spec);
void i2s_init_ADC(STRC_I2S_SPEC *i2s_spec, MMP_BOOL bResetDevice);
...
u32 I2S_DA32_GET_RP(void)
{
    ...
    dataLO = reg_read16(REG_IIS_OUT_RDPTR_LO);
    dataHI = reg_read16(REG_IIS_OUT_RDPTR_HI);
...
u32 I2S_DA32_GET_WP(void)
{
    ...
    dataLO = reg_read16(REG_IIS_OUT_WRPTR_LO);
    dataHI = reg_read16(REG_IIS_OUT_WRPTR_HI);
...
void I2S_DA32_SET_WP(u32 WP32)
{
    ...
    reg_write16(REG_IIS_OUT_WRPTR_LO, WP32 & 0xFFFF);
    reg_write16(REG_IIS_OUT_WRPTR_HI, WP32 >> 16);
}
...
u32 I2S_AD32_GET_RP(void)
{
    ...
    dataLO = reg_read16(REG_IIS_IN_RDPTR_LO);
    dataHI = reg_read16(REG_IIS_IN_RDPTR_HI);
...
u32 I2S_AD32_GET_WP(void)
{
    ...
    dataLO = reg_read16(REG_IIS_IN_WRPTR_LO);
    dataHI = reg_read16(REG_IIS_IN_WRPTR_HI);
...
void I2S_AD32_SET_RP(u32 RP32)
{
    reg_write16(REG_IIS_IN_RDPTR_LO, RP32 & 0xFFFF);
    reg_write16(REG_IIS_IN_RDPTR_HI, RP32 >> 16);
}
...
static void _i2s_power_on(void)
{
    ...
    if (_i2s_DA_running || _i2s_AD_running || _dev_running)
    {
        printf("I2S# power on already, DA:%d, AD:%d, Device:%d\n", _i2s_DA_running, _i2s_AD_running, _dev_running);
        return;
    }
...
static void _i2s_power_off(void)
{
    ...
    if (_i2s_DA_running || _i2s_AD_running || _dev_running)
    {
        printf("I2S# module still running, skip power-off, DA:%d, AD:%d, Device:%d\n", _i2s_DA_running, _i2s_AD_running, _dev_running);
        return;
    }
...
static void _i2s_reset(void)
{
    ...
    data16  = reg_read16(REG_AUDIO_CLOCK_REG_3E);
    data16 |= (1 << 12);
    reg_write16(REG_AUDIO_CLOCK_REG_3E, data16);
...
static void _i2s_aws_sync(void)
{
    ...
    /* AWS & ZWS sync */
    data16  = reg_read16(REG_IIS_ADC_SRATE_SET);
    data16 |= (1 << 15);
    reg_write16(REG_IIS_ADC_SRATE_SET, data16);
...
static void _i2s_set_clock(u32 demanded_sample_rate)
{
    ...
    if (_dev_running)
    {
        printf("I2S# Device is running, skip set clock !\n");
        return;
    }

    printf("I2S# %s, demanded_sample_rate: %u\n", __func__, demanded_sample_rate);

    if ((demanded_sample_rate > 45600) && (demanded_sample_rate < 50400))
    {
        target_sample_rate = 48000;
    }
    else if ((demanded_sample_rate > 41895) && (demanded_sample_rate < 46305))
    {
        target_sample_rate = 44100;
    }
    else if ((demanded_sample_rate > 30400) && (demanded_sample_rate < 33600))
    {
        target_sample_rate = 32000;
    }
    else if ((demanded_sample_rate > 22800) && (demanded_sample_rate < 25200))
    {
        target_sample_rate = 24000;
    }
    else if ((demanded_sample_rate > 20900) && (demanded_sample_rate < 23100))
    {
        target_sample_rate = 22050;
    }
    else if ((demanded_sample_rate > 15200) && (demanded_sample_rate < 16800))
    {
        target_sample_rate = 16000;
    }
    else if ((demanded_sample_rate > 11400) && (demanded_sample_rate < 12600))
    {
        target_sample_rate = 12000;
    }
    else if ((demanded_sample_rate > 10450) && (demanded_sample_rate < 11550))
    {
        target_sample_rate = 11025;
    }
    else if ((demanded_sample_rate > 7600) && (demanded_sample_rate < 8400))
    {
...
static void _i2s_enable_fading(u32 fading_step, u32 fading_duration)
{
    ...
    data16  = reg_read16(REG_IIS_OUT_FADE);
    data16 |= (1 << 12);
    data16 |= ((fading_step & 0xF) << 8);
    data16 |= (fading_duration & 0xFF);
    reg_write16(REG_IIS_OUT_FADE, data16);
}
    ...
    if (i2s_spec->use_hdmirx)    /* NOTE: i2s played the slave role */
    {
        if (i2s_spec->internal_hdmirx)
        {
            if (i2s_spec->use_hdmitx)
                reg_write16(REG_IIS_IO_SET, 0x3308);//kenny
            else
                reg_write16(REG_IIS_IO_SET, 0x00B8);

            if (!i2s_spec->slave_mode)
                reg_write16(REG_IIS_IOMUX_CTRL, 0x0000);  /* [9:8] pinZD0 select IO's ZDO_I */
            else
                reg_write16(REG_IIS_IOMUX_CTRL, 0x00F0); /* [9:8] pinZD0 select IO's ZDO_I, [7:4]select hdmirx i2s0-3 */
        }
        else                                              /* ext. hdmirx */
        {
            if (i2s_spec->use_hdmitx)
                reg_write16(REG_IIS_IO_SET, 0x00BF);
            else
                reg_write16(REG_IIS_IO_SET, 0x00BF);

            reg_write16(REG_IIS_IOMUX_CTRL, 0x0000);
        }
    }
    else     /* no use hdmirx */
    {
        if (i2s_spec->slave_mode)
        {
            reg_write16(REG_IIS_IO_SET, 0x00BB);
            reg_write16(REG_IIS_IOMUX_CTRL, 0x0200); /* [9:8] pinZD0 select IO's DSD_I */
        }
        else                                         /* i2s master (Wolfson & Cirrus slave mode) */
        {
            reg_write16(REG_IIS_IO_SET, 0x00B8);
            reg_write16(REG_IIS_IOMUX_CTRL, 0x0000);             /* [9:8] pinZD0 select IO's ZD0_I */
        }
    }
}
...
void i2s_pause_ADC(int pause)
{
    ...
    if (pause)
    {
        data16  = reg_read16(REG_IIS_IN_CTRL1);
        data16 &= ~(1 << 0);
        reg_write16(REG_IIS_IN_CTRL1, data16);
    }
    else     /* resume */
    {
        data16  = reg_read16(REG_IIS_IN_CTRL1);
...
void i2s_pause_DAC(int pause)
{
    ...
    if (pause)
    {
        ...
        reg_write16(REG_IIS_OUT_VOLUME, 0x0000);

        data16  = reg_read16(REG_IIS_OUT_CTRL2);
        data16 &= ~(1 << 0);
        reg_write16(REG_IIS_OUT_CTRL2, data16);
    }
    else                               /* resume */
    {
        _i2s_enable_fading(0x1, 0xFF); /* slow response */
        reg_write16(REG_IIS_OUT_VOLUME, 0xFF00);
...
void i2s_deinit_ADC(MMP_BOOL bDeInitDevice)
{
    ...
    if (bDeInitDevice && !_dev_running)
        printf("I2S# Device is 'NOT' running, skip deinit Device !\n");
...
void i2s_deinit_DAC(void)
{
    ...
    do
    {
        ...
        out_status_1 = reg_read16(REG_IIS_OUT_STATUS1);
        out_status_2 = reg_read16(REG_IIS_OUT_STATUS2);
...
//	u32 data32;
    ...
    if ((u32)i2s_spec->base_out_i2s_spdif % 8)
    {
        param_fail = 1;
    }
    if (i2s_spec->buffer_size % 8)
    {
        param_fail = 4;
    }
...
//	}
    ...
    {
        u16 data16_tmp;
        data16_tmp  = reg_read16(REG_IIS_ADC_SRATE_SET);
        data16_tmp &= ~(0x1FF << 0);                    /* [8:0] B0AWSRatio */
        data16_tmp |= (0x1F << 0);                      /* sample width (in bits unit) */
        reg_write16(REG_IIS_ADC_SRATE_SET, data16_tmp); /* zclk ratio=0x1F */
    }
    ...
    reg_write16(REG_IIS_DAC_SRATE_SET, data16);
    ...
    if (i2s_spec->is_big_endian)
    {
        ...
        {
            data16  = reg_read16(REG_IIS_CODEC_PCM_SET);
            data16 |= (1 << 3);
            reg_write16(REG_IIS_CODEC_PCM_SET, data16);
        }
        else
        {
            data16  = reg_read16(REG_IIS_CODEC_PCM_SET);
    ...
    {
        /* buf 1 (IIS/SPDIF) */
        reg_write16(REG_IIS_BASE_OUT1_LO, (u32)i2s_spec->base_out_i2s_spdif & 0xFFFF);
        reg_write16(REG_IIS_BASE_OUT1_HI, (u32)i2s_spec->base_out_i2s_spdif >> 16);

        /* SPDIF data channel select */
        {
            data16  = reg_read16(REG_IIS_SPDIF_VOL);
            data16 &= ~(1 << 14);
            reg_write16(REG_IIS_SPDIF_VOL, data16);
        }
    }

    /* buffer length */
    reg_write16(REG_IIS_OUT_LEN_LO, (i2s_spec->buffer_size - 1) & 0xFFFF);     /* NOTE: minus one for hardware design */
    reg_write16(REG_IIS_OUT_LEN_HI, (i2s_spec->buffer_size - 1) >> 16);

    /* DA interrupt threshold: (value = remnant data) to starvation */
    reg_write16(REG_IIS_OUT_GAP_LO, 0x0040);     /* TODO: need reasonable threshold */
    reg_write16(REG_IIS_OUT_GAP_HI, 0x0002);

    /* output path */
    {
        reg_write16(REG_IIS_OUT_CTRL1,
...
void i2s_init_ADC(STRC_I2S_SPEC *i2s_spec, MMP_BOOL bInitDevice)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if ((u32)i2s_spec->base_in_hdmi[i] % 8)
        {
            printf("ERROR# I2S, bufbase must be 8-Bytes alignment !\n");
            return;
        }
    }
    ...
    if (bInitDevice && _dev_running)
        printf("I2S# Device is running, skip re-init Device !\n");
    ...
    /* config AD */
    {
        ...
        data16_tmp  = reg_read16(REG_IIS_DAC_SRATE_SET);
        data16_tmp &= ~(1 << 12);
        data16_tmp |= (data16 & 0x1000);
        data16_tmp &= ~(0x1FF << 0);                    /* [8:0] B0ZWSRatio */
        data16_tmp |= (0x1F);                           /* sample width (in bits unit) */
        reg_write16(REG_IIS_DAC_SRATE_SET, data16_tmp); /* set 'master or slave mode' here ! */
    ...
    gtIsUseHdmiRx = i2s_spec->use_hdmirx;
    gtIsSlaveMode = i2s_spec->slave_mode;

#ifdef USE_WM8960_ADC
    if (bInitDevice && !_dev_running)
    {
        if (i2s_spec->use_hdmirx)
        {
            itp_codec_lock_dev();
            itp_codec_rec_init();
            itp_codec_unlock_dev();
    ...
    /* buffer base */
    {
        if (chipVer == 1)
        {
            reg_write16(REG_IIS_BASE_IN1_LO, (u32)i2s_spec->base_in_hdmi[0] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN1_HI, (u32)i2s_spec->base_in_hdmi[0] >> 16);

            reg_write16(REG_IIS_BASE_IN2_LO, (u32)i2s_spec->base_in_hdmi[1] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN2_HI, (u32)i2s_spec->base_in_hdmi[1] >> 16);

            reg_write16(REG_IIS_BASE_IN3_LO, (u32)i2s_spec->base_in_hdmi[2] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN3_HI, (u32)i2s_spec->base_in_hdmi[2] >> 16);

            reg_write16(REG_IIS_BASE_IN4_LO, (u32)i2s_spec->base_in_hdmi[3] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN4_HI, (u32)i2s_spec->base_in_hdmi[3] >> 16);

            reg_write16(REG_IIS_BASE_IN5_LO, (u32)i2s_spec->base_in_i2s & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN5_HI, (u32)i2s_spec->base_in_i2s >> 16);
        }
        else
        {
            reg_write16(REG_IIS_BASE_IN1_LO, (u32)i2s_spec->base_in_i2s & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN1_HI, (u32)i2s_spec->base_in_i2s >> 16);

            reg_write16(REG_IIS_BASE_IN2_LO, (u32)i2s_spec->base_in_hdmi[0] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN2_HI, (u32)i2s_spec->base_in_hdmi[0] >> 16);

            reg_write16(REG_IIS_BASE_IN3_LO, (u32)i2s_spec->base_in_hdmi[1] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN3_HI, (u32)i2s_spec->base_in_hdmi[1] >> 16);

            reg_write16(REG_IIS_BASE_IN4_LO, (u32)i2s_spec->base_in_hdmi[2] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN4_HI, (u32)i2s_spec->base_in_hdmi[2] >> 16);

            reg_write16(REG_IIS_BASE_IN5_LO, (u32)i2s_spec->base_in_hdmi[3] & 0xFFFF);
            reg_write16(REG_IIS_BASE_IN5_HI, (u32)i2s_spec->base_in_hdmi[3] >> 16);
        }
    }

    /* buffer length */
    reg_write16(REG_IIS_IN_LEN_LO, (i2s_spec->buffer_size - 1) & 0xFFFF);     /* NOTE: minus one for hardware design */
    reg_write16(REG_IIS_IN_LEN_HI, (i2s_spec->buffer_size - 1) >> 16);        /* NOTE: minus one for hardware design */

    /* AD interrupt threshold: (value = available space) to full */
    reg_write16(REG_IIS_IN_GAP_LO, 0x0040);     /* TODO: need reasonable threshold */
    reg_write16(REG_IIS_IN_GAP_HI, 0x0002);

    if (chipVer == 1)
    {
        if (i2s_spec->from_MIC_IN && i2s_spec->digital_IN)
        {
            reg_write16(REG_IIS_IN_CTRL2,
...
void codec_initialize(
    void)
{
    itp_codec_creat_semaphore();
    itp_codec_param_init();
}

void codec_terminate(
    void)
{
    itp_codec_delete_semaphore();
}
    ...
    unsigned int isAnalogAudioDev)
{
    ...
    switch (inType)
    {
    case CODEC_INPUT_ONLY_MIC:
        itp_codec_set_linein(1, 0, 0);
        break;

    case CODEC_INPUT_ONLY_LINE:
        itp_codec_set_linein(0, (!isAnalogAudioDev) & 0x1, isAnalogAudioDev & 0x1);
        break;

    case CODEC_INPUT_MIC_LINE:
        itp_codec_set_linein(1, (!isAnalogAudioDev) & 0x1, isAnalogAudioDev & 0x1);
        break;

    case CODEC_INPUT_NONE:
        itp_codec_set_linein(0, 0, 0);
        break;
    }
...
#ifdef USE_WM8960_ADC
    itp_codec_lock_dev();
    itp_codec_set_boost_gain(0, boostgain);
    itp_codec_unlock_dev();
#endif
}
...
#ifdef USE_WM8960_ADC
    itp_codec_lock_dev();
    itp_codec_rec_set_direct_vol(vol);
    itp_codec_unlock_dev();
#endif
}
...
#ifdef USE_WM8960_ADC
    itp_codec_lock_dev();
    itp_codec_set_boost_gain(line, boostgain);
    itp_codec_unlock_dev();
#endif
}