...
BYTE HDMIRX_ReadI2C_Byte(BYTE RegAddr)
{
    ...
    AHB_WriteRegisterMask(GPIO_BASE + 0xD0, (INTERNAL_IIC_MODE << 28), (0x3 << 28));
            
    if (0 != (flag = mmpIicReceiveData(IIC_MASTER_MODE, IT6607_IICADDR, RegAddr, &d, 1)))
    {
    ...
    AHB_WriteRegisterMask(GPIO_BASE + 0xD0, (EXTERNAL_IIC_MODE << 28), (0x3 << 28));
...
SYS_STATUS HDMIRX_WriteI2C_Byte(BYTE RegAddr, BYTE d)
{
    ...
    if (0 != (flag = mmpIicSendData(IIC_MASTER_MODE, IT6607_IICADDR, RegAddr, &d, 1)))
    {
...
SYS_STATUS HDMIRX_ReadI2C_ByteN(BYTE RegAddr,BYTE *pData,int N)
{
    ...
    if (0 != (flag = mmpIicReceiveData(IIC_MASTER_MODE, IT6607_IICADDR, RegAddr, pData, N)))
    {
...
SYS_STATUS HDMIRX_WriteI2C_ByteN(BYTE RegAddr,BYTE *pData,int N)
{
    ...
    if ( 0 != (flag = mmpIicSendData(IIC_MASTER_MODE, IT6607_IICADDR, RegAddr, pData , N)))
    {
...
SYS_STATUS HDMIRX_SetI2C_Byte(SHORT RegAddr, BYTE Mask, BYTE Value)
{
    ...
    if( Mask != 0xFF )
    {
        for( i = 0; i < 3; i++ )
        {
            FLAG = mmpIicReceiveData(IIC_MASTER_MODE, IT6607_IICADDR ,RegAddr, &p_data, 1);
            if( FLAG == 0 ) break;
    ...
    for( i = 0; i < 3; i++ )
    {
        FLAG = mmpIicSendData(IIC_MASTER_MODE, IT6607_IICADDR, RegAddr, &p_data , 1);
        if(FLAG == 0) break;
        printf("RX Read I2C ERROR !!!");
