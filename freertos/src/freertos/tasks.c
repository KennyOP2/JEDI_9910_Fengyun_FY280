...
#define EXTENSION_FOR_SMEDIA
#if defined(EXTENSION_FOR_SMEDIA)
#  include "timer.h"
#  include "spr_defs.h"
#endif // EXTENSION_FOR_SMEDIA
...
typedef struct tskTaskControlBlock
{
    ...
    #if ( INCLUDE_vTaskRestart == 1)
        unsigned portSHORT      usStackDepth;   /*< Store the stack depth for task restart */
    #endif
    ...
    #if ( configUSE_TRACE_FACILITY == 1 ) || ( configUSE_DUMP_FACILITY == 1 )
...
/*
 * The mapping table between task name and task number.
 */
#if ( configUSE_DUMP_FACILITY == 1 )
signed portCHAR * pxTCBName[1024];
#endif
...
static unsigned portBASE_TYPE uxTaskNumber                      = ( unsigned portBASE_TYPE ) 0;
    ...
    static void prvGenerateRunTimeStatsForTasksInList( const signed portCHAR *pcWriteBuffer, const signed portCHAR mode, xList *pxList, unsigned portLONG ulTotalRunTime );
    static void prvInitRunTimeStatsForTasksInList( xList *pxList );
    static void prvGenerateCodeStackStatisticsInList(xList *pxList);
#endif

#if defined(EXTENSION_FOR_SMEDIA)
static volatile portBASE_TYPE xTickCountOverflowed              = pdFALSE;
#endif // EXTENSION_FOR_SMEDIA
...
#if defined(EXTENSION_FOR_SMEDIA) && ( INCLUDE_vTaskRestart == 1 )
signed portBASE_TYPE xTaskRestart( xTaskHandle pxTaskHandle, pdTASK_CODE pvTaskCode, void *pvParameters)
{
    if (pxTaskHandle != NULL)
    {
        portSTACK_TYPE *pxTopOfStack;
        tskTCB * pxNewTCB = (tskTCB *)pxTaskHandle;
        ...
        #if portSTACK_GROWTH < 0
        {
            pxTopOfStack = pxNewTCB->pxStack + ( pxNewTCB->usStackDepth - 1 ) - ( ( pxNewTCB->usStackDepth - 1 ) % portBYTE_ALIGNMENT );
        }
        #else
        {
...
#endif // defined(EXTENSION_FOR_SMEDIA)
...
tskTCB * pxNewTCB;
    ...
    if( pxNewTCB != NULL )
    {
        ...
        /* Stores the stack depth for task restarting latter */
        #if ( INCLUDE_vTaskRestart == 1)
        ...
        portENTER_CRITICAL();
        {
            ...
            #if ( configUSE_DUMP_FACILITY == 1 )
            pxTCBName[uxTaskNumber] = pxNewTCB->pcTaskName;
            #endif
...
/*-----------------------------------------------------------*/
    ...
    tskTCB *pxTCB;
        ...
        taskENTER_CRITICAL();
        {
            ...
            /* Increment the uxTaskNumber Variable also so kernel aware debuggers
...
#if defined(ENABLE_SXA_MPS) && (ENABLE_SXA_MPS) && defined(EXTENSION_FOR_SMEDIA)
int gnDisableTaskSwitch = 0;
#endif // defined(ENABLE_SXA_MPS) && (ENABLE_SXA_MPS) && defined(EXTENSION_FOR_SMEDIA
...
portBASE_TYPE xReturn;
    ...
    xReturn = xTaskCreate( prvIdleTask, ( signed portCHAR * ) "IDLE", 1024 /*tskIDLE_STACK_SIZE*/, ( void * ) NULL, tskIDLE_PRIORITY, ( xTaskHandle * ) NULL );
...
#if defined(EXTENSION_FOR_SMEDIA)
portTickType xTaskGetTimerCount( void )
{
portTickType xTicks;
...
// Call this function if the clock period is changed to update the global setting.
void xTaskTimerUpdate( void )
{
    /* Critical section required if running on a 16 bit processor. */
    taskENTER_CRITICAL();
    {
...
portTickType xTicks;
    ...
    {
#if defined(EXTENSION_FOR_SMEDIA)
        xTicks = timer_get_ticks();

        if (xTicks < xTickCount)
        {
            xTickCountOverflowed = pdTRUE;
        }
...
#if defined(EXTENSION_FOR_SMEDIA)
    void vTaskInitRunTimeStats( void )
    {
    unsigned portBASE_TYPE uxQueue;

        // re-initialize the timer for runtime stats
        portCONFIGURE_TIMER_FOR_RUN_TIME_STATS();
        ...
        vTaskSuspendAll();
        {
            ...
            do
            {
                ...
                if( !listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) )
                {
                    prvInitRunTimeStatsForTasksInList( ( xList * ) &( pxReadyTasksLists[ uxQueue ] ) );
                }
            }while( uxQueue > ( unsigned portSHORT ) tskIDLE_PRIORITY );

            if( !listLIST_IS_EMPTY( pxDelayedTaskList ) )
            {
                prvInitRunTimeStatsForTasksInList( ( xList * ) pxDelayedTaskList );
            }

            if( !listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) )
            {
                prvInitRunTimeStatsForTasksInList( ( xList * ) pxOverflowDelayedTaskList );
            }
            ...
            #if ( INCLUDE_vTaskDelete == 1 )
            {
                if( !listLIST_IS_EMPTY( &xTasksWaitingTermination ) )
                {
                    prvInitRunTimeStatsForTasksInList( ( xList * ) &xTasksWaitingTermination );
                }
            }
            #endif
            ...
            #if ( INCLUDE_vTaskSuspend == 1 )
            {
                if( !listLIST_IS_EMPTY( &xSuspendedTaskList ) )
                {
                    prvInitRunTimeStatsForTasksInList( ( xList * ) &xSuspendedTaskList );
                }
            }
            #endif
        }
    ...
    unsigned portLONG ulTotalRunTime = portGET_RUN_TIME_COUNTER_VALUE();
        ...
        vTaskSuspendAll();
        {
            ...
            do
            {
                ...
                if( !listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) )
                {
                    prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, tskREADY_CHAR, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), ulTotalRunTime );
                }
            }while( uxQueue > ( unsigned portSHORT ) tskIDLE_PRIORITY );

            if( !listLIST_IS_EMPTY( pxDelayedTaskList ) )
            {
                prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, tskBLOCKED_CHAR, ( xList * ) pxDelayedTaskList, ulTotalRunTime );
            }

            if( !listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) )
            {
                prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, tskBLOCKED_CHAR, ( xList * ) pxOverflowDelayedTaskList, ulTotalRunTime );
            }
            ...
            #if ( INCLUDE_vTaskDelete == 1 )
            {
                if( !listLIST_IS_EMPTY( &xTasksWaitingTermination ) )
                {
                    prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, tskDELETED_CHAR, ( xList * ) &xTasksWaitingTermination, ulTotalRunTime );
                }
            }
            #endif
            ...
            #if ( INCLUDE_vTaskSuspend == 1 )
            {
                if( !listLIST_IS_EMPTY( &xSuspendedTaskList ) )
                {
                    prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, tskSUSPENDED_CHAR, ( xList * ) &xSuspendedTaskList, ulTotalRunTime );
                }
            }
            #endif
            ...
            {
                printf( "\r\nEstimation period: %u ms\r\n", ulTotalRunTime / 1000 );
            }
        }
        xTaskResumeAll();
    }

    void vTaskGetRunCodeStackStats( signed portCHAR *pcWriteBuffer )
    {
    unsigned portBASE_TYPE uxQueue;
        ...
        vTaskSuspendAll();
        {
            ...
            {
                puts("\r\n");
                puts("         Task   Depth   Stack Size  Remain Stack Size");
                puts("============= ======= ============ ==================");
            }
            ...
            do
            {
                ...
                if( !listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) )
                {
                    prvGenerateCodeStackStatisticsInList(( xList * ) &( pxReadyTasksLists[ uxQueue ]));
            ...
            if( !listLIST_IS_EMPTY( pxDelayedTaskList ) )
            {
                prvGenerateCodeStackStatisticsInList(( xList * ) pxDelayedTaskList);
            }

            if( !listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) )
            {
                prvGenerateCodeStackStatisticsInList(( xList * ) pxOverflowDelayedTaskList);
            }
            ...
            #if ( INCLUDE_vTaskDelete == 1 )
            {
                if( !listLIST_IS_EMPTY( &xTasksWaitingTermination ) )
                {
                    prvGenerateCodeStackStatisticsInList(( xList * ) &xTasksWaitingTermination);
                }
            }
            #endif
            ...
            #if ( INCLUDE_vTaskSuspend == 1 )
            {
                if( !listLIST_IS_EMPTY( &xSuspendedTaskList ) )
                {
                    prvGenerateCodeStackStatisticsInList(( xList * ) &xSuspendedTaskList);
                }
            }
            #endif
...
#if 0 // Enable it to print the info of contex-switch
    {
        static unsigned char uxPreviousTaskName[256] = "IDLE";

        if( strcmp(uxPreviousTaskName, pxCurrentTCB->pcTaskName) )
        {
            printf("[TASK SWITCH] %dms: switch task (%s) to task (%s)\r\n", xTaskGetTimerCount(), uxPreviousTaskName, pxCurrentTCB->pcTaskName);
            strcpy(uxPreviousTaskName, pxCurrentTCB->pcTaskName);
        }
    }
#endif
}
...
#if defined(EXTENSION_FOR_SMEDIA)
char *vGetCurrentTaskName(void)
{
    return pxCurrentTCB->pcTaskName;
}

#if ( configUSE_TRACE_FACILITY == 1 ) || ( configUSE_DUMP_FACILITY == 1 )
unsigned portBASE_TYPE vGetCurrentTaskID(void)
{
    return pxCurrentTCB->uxTCBNumber;
}

unsigned portBASE_TYPE vGetMaxTaskID(void)
{
    return uxTaskNumber;
}
#endif // ( configUSE_TRACE_FACILITY == 1 ) || ( configUSE_DUMP_FACILITY == 1 )
#endif // defined(EXTENSION_FOR_SMEDIA)
...
#if 0
    unsigned portBASE_TYPE xReadyPriority = configMAX_PRIORITIES-1;
    while( listLIST_IS_EMPTY( &( pxReadyTasksLists[ xReadyPriority ] ) ) )
    {
        --xReadyPriority;
        if( xReadyPriority == tskIDLE_PRIORITY)
        {
            vTaskCheckDelayedTasks();
            xReadyPriority = configMAX_PRIORITIES-1;
        }
    }
#else

    int i;
    for (i=0;i<55000;i++) { asm(""); }
#endif
...
/*-----------------------------------------------------------*/
    ...
    unsigned portSHORT usStackRemaining;
        ...
        do
        {
            ...
            sprintf( pcStatusString, ( portCHAR * ) "%-16s %c\t\t%2u\t%5u\t%2u\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber );
            if ( pcWriteBuffer != NULL )
            {
            ...
            {
                printf( "%s", ( portCHAR * ) pcStatusString );
            }
...
/*-----------------------------------------------------------*/
    ...
    portCHAR unit[3] = "ms";
        ...
        do
        {
            ...
            if( ulTotalRunTime > 1000UL )
            {
                /* Has the task run at all? */
                if( pxNextTCB->ulRunTimeCounter == 0 )
                {
                    /* The task has used no CPU time at all. */
                    sprintf( pcStatsString, ( portCHAR * ) "%-16s %c %7uus\t%2u%%\r\n", pxNextTCB->pcTaskName, mode, 0, 0 );
                }
                else
                {
                    unsigned int normalize_time;

                    /* What percentage of the total run time as the task used?
                    This will always be rounded down to the nearest integer. */
                    // ulStatsAsPercentage = ( 100UL * (pxNextTCB->ulRunTimeCounter / 1000) ) / (ulTotalRunTime / 1000);
                    ulStatsAsPercentage = ( pxNextTCB->ulRunTimeCounter / 10 ) / (ulTotalRunTime / 1000);

                    if( pxNextTCB->ulRunTimeCounter > 1000UL )
                    {
                        normalize_time = pxNextTCB->ulRunTimeCounter / 1000;
                        unit[0] = 'm';
                    }
                    else
                    {
                        normalize_time = pxNextTCB->ulRunTimeCounter;
                        unit[0] = 'u';
                    }

                    if( ulStatsAsPercentage > 0UL )
                    {
                        sprintf( pcStatsString, ( portCHAR * ) "%-16s %c %7u%s\t%2u%%\r\n", pxNextTCB->pcTaskName, mode, normalize_time, unit, ( unsigned int ) ulStatsAsPercentage );
                    }
                    else
                    {
                        /* If the percentage is zero here then the task has
                        consumed less than 1% of the total run time. */
                        sprintf( pcStatsString, ( portCHAR * ) "%-16s %c %7u%s\t<1%%\r\n", pxNextTCB->pcTaskName, mode, normalize_time, unit );
                    }
                }
                ...
                {
                    printf( "%s", ( portCHAR * ) pcStatsString );
                }
            }
    ...
    static void prvGenerateCodeStackStatisticsInList(xList *pxList)
    {
        ...
        do
        {
            ...
            for (i = 0; i < pxNextTCB->usStackDepth; i++)
            {
                if (pxNextTCB->pxStack[i] != 0xa5a5a5a5)
                    break;
            }
            printf("%-13s %7u %12u %18u\r\n",
                   pxNextTCB->pcTaskName,
                   pxNextTCB->usStackDepth,
                   (pxNextTCB->usStackDepth * sizeof(portSTACK_TYPE)),
                   i * sizeof(portSTACK_TYPE));
        } while( pxNextTCB != pxFirstTCB );
    }
#endif
/*-----------------------------------------------------------*/
...
register tskTCB *pxTCB;
    ...
    if (pdTRUE == xTickCountOverflowed)
    {
        volatile xList *pxTemp;

        /* Wake up every task in pxDelayedTaskList. There should not be
           any task there.
        */
        ...
        pxTemp = pxDelayedTaskList;
        pxDelayedTaskList = pxOverflowDelayedTaskList;
        pxOverflowDelayedTaskList = (xList *) pxTemp;

        xTickCountOverflowed = pdFALSE;
    }
